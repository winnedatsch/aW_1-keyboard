...
class Battery {
	public:
		 ...
		 * @param adcDevice is the zephyr ADC device that the sensePin is bound to
		 * @param sensePin is the analog pin used for sensing the battery voltage
		 */
		Battery(uint16_t minVoltage, uint16_t maxVoltage, device *adcDevice, uint8_t sensePin);
	...
	private:
		const adc_channel_cfg channelConfig;
		device *adcDevice;
		uint16_t minVoltage;
		uint16_t maxVoltage;
		uint16_t refVoltage;
